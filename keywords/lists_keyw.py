...
def index_keyword(op : Oper, state, local : Optional[dict[str, object]]):
    list : List | str
    index : int
...
def setindex_keyword(op : Oper, state, local : Optional[dict[str, object]]):
...
def append_keyword(op : Oper, state, local : Optional[dict[str, object]]):
    binarian_assert(state.is_expr, "This operation is unavailable in expressions.", state)
